...
int **refMatrix = NULL; //pointer to pointer
  ...
  while (matrix_downloaded == false) {
    if (myTransfer.available()) {
      ...
      if (rxRow == 0) {
        ...
        refMatrix = (int **)malloc(nRow * sizeof(int*));

        // Allocate memory for column
        for (iRow = 0 ; iRow < nRow ; iRow++) {
          refMatrix[iRow] = (int *)malloc(nColumn * sizeof(int));

          // Check memory validity
          if (refMatrix[iRow] == NULL) {
            //freeAllocatedMemory(refMatrix, iRow);
          }
        }

        refMatrix[0][0] = (int)motorStruct.m1;
        refMatrix[0][1] = (int)motorStruct.m2;
        refMatrix[0][2] = (int)motorStruct.m3;
        refMatrix[0][3] = (int)motorStruct.m4;
        refMatrix[0][4] = (int)motorStruct.m5;
        refMatrix[0][5] = (int)motorStruct.m6;
        rxRow++;
        //matrix_downloaded = true;

      } else if (rxRow > 0) {
        refMatrix[rxRow][0] = (int)motorStruct.m1;
        refMatrix[rxRow][1] = (int)motorStruct.m2;
        refMatrix[rxRow][2] = (int)motorStruct.m3;
        refMatrix[rxRow][3] = (int)motorStruct.m4;
        refMatrix[rxRow][4] = (int)motorStruct.m5;
        refMatrix[rxRow][5] = (int)motorStruct.m6;
        rxRow++;
      }
    }
...
//    Serial.println(counter2);
  ...
  for (i=0;i<6;++i){
      error[i]=refMatrix[number][i]-sensor[i];
      }
  i = 0;
